...
log4js.configure({
	appenders: {
		cons: { type: 'console' },
		server: { type: 'file', filename: 'C:\\Users\\stric\\Desktop\\serverDoorbell.log' }
	},
	categories: {
...
var checkVolume = function () {
	logger.debug('initial volume', globalVolume, globalDoorbellVolume, globalCatVolume)
        try {
	...
	try {
		parseInt(globalCatVolume)
	} catch {
		logger.debug('setting Cat to default 50')
		globalCatVolume = '50'
	}
...
var getVolumes = function(volName) {
	...
	var req = http.request(volOptions, res => {
	  ...
	  res.on('data', d => {
	    ...
	    if (volName == 'rpiCatVolume') globalCatVolume += d 
...
app.post('/api/doorbell/', function(req,res){
        logger.debug('file post' + JSON.stringify(req.body))
        if (req.body && req.body.file){
	   var path = "\\\\192.168.0.190\\big\\Music\\Doorbell\\" + req.body.file
	   if (!fs.existsSync(path)) {
		   logger.debug("file doesnt exist " + path)
		   path = "\\\\192.168.0.190\\big\\Music\\Doorbell\\" + req.body.file
	   }
            stopMplayer()
...
var startMplayer = function(file, additionalParams){
	...
	mplayerArgs.push('\\\\192.168.0.190\\big\\Music' + file.substring(25).replace(/\//g, "\\"))
