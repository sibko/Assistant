...
log4js.configure({
	appenders: {
		cons: { type: 'console' },
		server: { type: 'file', filename: 'C:\\Users\\stric\\Desktop\\serverCat.log' }
	},
	categories: {
...
app.post('/api/doorbell/', function(req,res){
        logger.debug('file post' + JSON.stringify(req.body))
        if (req.body && req.body.file){
	   var path = "//music/Doorbell/" + req.body.file
...
var startMplayer = function(file, additionalParams){
	...
	mplayerArgs.push('\\\\192.168.0.190\\big\\Music' + file.substring(7).replace(/\//g, "\\"))
